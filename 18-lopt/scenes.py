...
class Farmer(MovingCameraScene):
    def construct(self):
        ...
        points[3][0][11].set_opacity(0)
        points[3][0][-1].set_opacity(0)
        points[3][0][11].set_color(BLACK)
        points[3][0][-1].set_color(BLACK)
        ...
        exp = Tex(r"$$\max\ " + str(OF_INITIAL[0]) + r"x_p + " + str(OF_INITIAL[1]) + r" x_c$$")
        ...
        #def optimum_updater(obj):
        #    vector  = of_arrow_shadow.get_end() - of_arrow_shadow.get_start()
        #    x, y, _ = vector

        #    xvar, yvar, _ = (optimum.get_center() - of_arrow_shadow.get_start())
        #    xval = str(int(round(xvar)) * 1000)
        #    yval = str(int(round(yvar)) * 1000)

        #    # HACK
        #    if xval == "2000":
        #        return

        #    profit = str(int(x * (xvar * 1000) + y * (yvar * 1000)))

        #    def formnum(x):
        #        return x[:-3] + r"\," + x[-3:]

        #    # NOTE: this prevents a memory leak
        #    # I'm not sure why, but simply doing becomes leaks the highlight objects
        #    # which accumulate in the TeX objects...
        #    obj[0].remove(obj[0][1].set_opacity(0))
        #    obj[1].remove(obj[1][1].set_opacity(0))
        #    obj[2].remove(obj[2][1].set_opacity(0))

        #    # NOTE: copy-pasted
        #    obj[0].become(
        #        Tex("$" + formnum(xval) + r"\ \mathrm{kg}$").scale(0.8)\
        #                .move_to(optimum).align_to(numberplane.axes[0], UP).shift(DOWN * 0.35)\
        #                .set_z_index(100000)
        #    )

        #    obj[1].become(
        #        Tex("$" + formnum(yval) + r"\ \mathrm{kg}$").scale(0.8)\
        #                .move_to(optimum).align_to(numberplane.axes[1], RIGHT).shift(LEFT * 0.35)\
        #                .set_z_index(100000),
        #    )
        ...
        #    obj[2].become(
        #        Tex(r"$\$" + formnum(profit) + "$").scale(0.8)\
        #                .next_to(end, UP + RIGHT)\
        #                .set_z_index(100000),
        #    )

        #    for i in range(3):
        #        obj[i].add(CreateSR(obj[i]))

        #optimum_updater(optimum_texts)
        ...
        nb = Tex("non-basic").scale(0.7).set_z_index(rect_large.get_z_index() + 1).set_opacity(0).move_to(nonbasic)
        bb = Tex("basic").scale(0.7).set_z_index(rect_large.get_z_index() + 1).set_opacity(0).move_to(basic)
        ...
        self.play(
            Succession(
                Wait(0.25),
                AnimationGroup(
                    FadeIn(ineqs_base, exp_base, var_base, of_base, shift=DOWN * self.camera.frame.height),
                    FadeOut(ineqs8, exp6, nonbas_vars, bas_vars, of3, nonbasic, basic, shift=DOWN * self.camera.frame.height),
                    run_time=1.5,
                ),
            ),
            Succession(
        ...
        self.play(
            FadeOut(t[0][-5:], shift=DOWN * 0.25),
            FadeIn(t2[0][-4:], shift=DOWN * 0.25),
        )
...
class Test3D(ThreeDScene):
    def construct(self):
        ...
        additional_ineqs = []
        # TODO this should be WRT the vector of the edge
...
class TransparentFunnyXD(MovingCameraScene):
    ...
    def construct(self):
        ...
        ltext.add(SurroundingRectangle(ltext, color=WHITE, fill_opacity=1, fill_color=BLACK, corner_radius=0.2, buff=0.3))

        rtext = Tex("Sir this is a McDonalds.").set_z_index(10)
        rtext.add(SurroundingRectangle(VGroup(rtext, ltext[0][4]), color=WHITE, fill_opacity=1, fill_color=BLACK, corner_radius=0.2, buff=0.3))

        rtext2 = Tex("Security!").set_z_index(10)
        rtext2.add(SurroundingRectangle(rtext2, color=WHITE, fill_opacity=1, fill_color=BLACK, corner_radius=0.2, buff=0.3))
...
class ILP(MovingCameraScene):
    ...
    def construct(self):
        ...
        fade_rect = get_fade_rect(opacity=1 - BIG_OPACITY / 1.2)
...
class BinaryHint(MovingCameraScene):
    @fade
    def construct(self):
        a = Tex("$b_1$")
        b = Tex(r"$1 \ge b_1$")
        ...
        sr = SurroundingRectangle(VGroup(a, b, c), fill_color=BLACK, stroke_color=WHITE, buff=0.35)
        sr2 = SurroundingRectangle(d, fill_color=BLACK, stroke_color=WHITE, buff=0.35)
...
class Knapsack(MovingCameraScene):
    def construct(self):
        ...
        self.play(
            FadeTransform(var, var_s),
            FadeTransform(ineqs, ineqs_s),
            FadeTransform(exp, exp_s),
        )
        ...
        self.play(FadeIn(sr), FadeIn(sr2))
...
class Outro(MovingCameraScene):
    def construct(self):
        ...
        att = VGroup(
            ...
            Tex(r"\it Inifinite loop?").scale(subtext_scale),
        ).arrange(DOWN, buff=0.1).next_to(at, DOWN, buff=0.3).set_color(RED)
        btt = VGroup(
            Tex(r"\it Prove the theorems.").scale(subtext_scale),
            Tex(r"\it Fast algorithms?").scale(subtext_scale),
        ).arrange(DOWN, buff=0.15).next_to(bt, DOWN, buff=0.3).set_color(RED)
        ctt = Tex(r"\it Approximations?").scale(subtext_scale).next_to(ct, DOWN, buff=0.3).set_color(RED)

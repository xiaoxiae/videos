    ...
    while len(queue) != 0:
        ...
        for next_state in next_states(*current, blueprint):
            if next_state not in visited:
                if not can_be_best(*next_state, max_geodes):
                    continue

                heapq.heappush(queue, (ores[0] * 1000000 + ores[1] * 10000 + ores[2] * 100 + ores[3], *next_state))
